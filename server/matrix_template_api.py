...
class MatrixTemplateService:
    ...
    def _preview_needs_library(self, payload: dict) -> bool:
        """预览只在选材真的会打到素材库时才要求素材库就绪。"""
        if payload["bgm"] and payload.get("template_id") not in (
            set(FIXED_SKILL_TEMPLATE_CONFIGS) | {NINE_GRID_TEMPLATE_ID}
        ):
            return True
        if payload.get("material_policy", "shared") == "shared":
            return True
        selected = self._user_materials(payload)
        return selected is None or len(selected) < self.required_visuals(payload)
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path == "/v1/preflight":
                ...
                # 参数微调（§3.6）：按有效布局重测断句/安全区，返回 effective_overrides
                # 与几何/对比度校验；语义缓存只复用不重写。
