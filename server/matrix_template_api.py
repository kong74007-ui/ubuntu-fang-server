...
try:
    ...
    import matrix_motion_v3 as motion_v3
...
class JobStore:
    ...
    def update_material_content_sha256(
        ...
        if not SHA_RE.fullmatch(digest):
            raise MatrixTemplateError("素材内容哈希无效")
        with self.connect() as db:
            db.execute("BEGIN IMMEDIATE")
            row = db.execute(
...
class MatrixTemplateService:
    ...
    def _load_fixed_skill_template(self, template_id: str) -> dict:
        ...
        record = {
            ...
            "required_visuals_max": 22 if template_id == motion_v3.BILINGUAL else config["required_visuals"],
            "clip_duration_range_seconds": [
    ...
    def library_readiness(self, *, force: bool = False) -> dict:
        with self.library_readiness_lock:
            ...
            try:
                ...
                if any(t in getattr(self, "fixed_skill_templates", {}) for t in motion_v3.IDS):
                    maximum_clip = payload.get("max_clip_duration_seconds")
                    maximum_scenes = payload.get("max_selection_scenes")
                    ready = ready and (
                        type(maximum_clip) in (int, float) and math.isfinite(maximum_clip) and maximum_clip >= 5.1
                        and type(maximum_scenes) is int and maximum_scenes >= 22
                    )
                result = {
                    "ready": ready,
                    "selection_contract_version": (
    ...
    def _material_scenes(self, payload: dict) -> tuple[list[dict], int, bool]:
        ...
        scenes = [{
            ...
            "media_type": (
                "video"
                if index == 1 or reference_template
                else "visual"
            ),
