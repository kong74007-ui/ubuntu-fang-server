...
export function compileOverlayComponent(context) {
  ...
  const items = steps.map((item, index) => `<li id="${root}_step_${index + 1}" data-public-target="steps" data-safe-text="${safeTextAttribute(item)}"><span></span><i aria-hidden="true">${index + 1}</i></li>`).join("");
  ...
  return overlayResult({html, publicTargets: ["root", "progress", "steps", "current", "total"], textFit: value.textFit, fallback: "compact_steps", safeMaximums: value.safeMaximums});
}
