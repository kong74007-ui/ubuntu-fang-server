...
export function compileStepsStack(input) {
  ...
  const bodies = {
    ...
    numbered_cards: `<section class="hf-v2-card-process"><header class="hf-v2-card-counter clip" data-v2-region="counter" data-safe-text="${stepCount}" ${clipAttributes(prepared.duration, 2)}><span></span></header>${steps}<footer>${accent}</footer></section>`,
