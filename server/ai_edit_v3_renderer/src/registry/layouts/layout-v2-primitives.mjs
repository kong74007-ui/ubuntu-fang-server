...
export function assertLayoutInput(contract, {variantId, ratio, idPrefix, durationMs, slots, designTokens}) {
  ...
  return Object.freeze({prefix, duration: seconds(durationMs), slots, style: styleFromTokens(designTokens)});
}

export function assetOrFallback({prefix, slot, value, duration, trackIndex}) {
  const attrs = clipAttributes(duration, trackIndex);
  if (!value) return `<div id="${prefix}_${slot}" class="hf-v2-slot hf-v2-fallback clip" data-slot="${slot}" data-fallback="no_optional_media" data-fallback-state="rendered" ${attrs}><svg viewBox="0 0 100 100" aria-hidden="true" focusable="false"><circle cx="50" cy="50" r="38"></circle><path d="M28 62 43 47l11 10 18-20"></path></svg></div>`;
  ...
  return `<div id="${prefix}_${slot}" class="hf-v2-slot hf-v2-slot-media clip" data-slot="${slot}" ${attrs}>${element}</div>`;
}
...
export function speakerSlot({prefix, value, duration, trackIndex}) {
  ...
  const element = asset.kind === "video"
    ? `<video muted playsinline preload="metadata" src="${asset.path}"></video>`
    : `<img alt="" src="${asset.path}">`;
  return `<div id="${prefix}_speaker" class="hf-v2-speaker clip" data-slot="speaker" ${attrs}>${element}</div>`;
}
...
export function stepsSlot({prefix, value, duration, trackIndex}) {
  ...
  const items = value.items.map((item, index) => `<li data-step-index="${index}"><span data-safe-text="${escapeAttribute(assertSafeText(item, {maxChars: 80, maxLines: 1}))}"></span></li>`).join("");
  return `<ol id="${prefix}_steps" class="hf-v2-steps clip" data-slot="steps" ${clipAttributes(duration, trackIndex)}>${items}</ol>`;
}
...
export function layoutResult({contract, variantId, ratio, input, structure, body, criticalRegions}) {
  ...
  const html = `<section id="${root}" class="hf-v2-layout hf-v2-layout-${contract.id} clip" data-layout-v2="${contract.id}" data-layout-variant="${variantId}" data-layout-structure="${structure}" data-start="0" data-duration="${input.duration}" data-track-index="1"${input.style}>${body}<aside id="${safe}" class="hf-v2-safe-area clip" data-safe-area="${ratio}" ${clipAttributes(input.duration, 20)}></aside></section>`;
  return Object.freeze({
    html,
