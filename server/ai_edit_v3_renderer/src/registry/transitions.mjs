...
export function applyTransition({timeline, transition, outgoing, incoming, identity, operationVersion = "2.0", boundaryMs, sceneDurationMs, fps}) {
  ...
  if (identity !== undefined && identity !== null) throw new Error("transition_identity_unproven");
  ...
  const operations = operationVersion === "2.0"
    ? representativeOperations({transition, effectiveTransition, outgoing, incoming, startMs, durationMs})
    : null;
  if (operations) OPERATION_CONTEXTS.set(operations, Object.freeze({
    operationVersion, transition, effectiveTransition, outgoing, incoming, startMs, durationMs, sceneDurationMs,
  }));
  const audit = Object.freeze({
    operationVersion, transition, effectiveTransition, fallbackReason, boundaryMs, startMs, endMs: startMs + durationMs,
    durationMs, identityRequired: transition === "card_match_cut", ...(operations ? {operations} : {}),
  });
  if (operations) validateTransitionOperations(operations, {allowedTargets: new Set([outgoing, incoming]), sceneDurationMs, requireTrusted: true});
  applyCalls(timeline, audit, outgoing, incoming);
  return audit;
}

export function compileTransitionScript({transition, effectiveTransition, outgoing, incoming, startMs, durationMs, operationVersion = "1.0", operations}) {
  ...
  if (operations) {
    validateTransitionOperations(operations, {
      allowedTargets: new Set([outgoing, incoming]), requireTrusted: true,
      compileContext: {operationVersion, transition, effectiveTransition, outgoing, incoming, startMs, durationMs},
    });
...
function applyCalls(timeline, audit, outgoing, incoming) {
  ...
  if (audit.operations) {
    for (const operation of audit.operations) {
      ...
      else timeline.fromTo(operation.target, operation.from, {...operation.to, duration: operation.duration_ms / 1000, ease: operation.ease}, operation.start_ms / 1000);
    }
    return;
  }
...
function representativeOperations({transition, effectiveTransition, outgoing, incoming, startMs, durationMs}) {
  ...
  if (transition === "card_match_cut" && effectiveTransition === "soft_wipe") {
    return freezeOperations([{kind: "from_to", target: incoming, start_ms: startMs, duration_ms: durationMs, from: {clipPath: "inset(0 100% 0 0)"}, to: {clipPath: "inset(0 0% 0 0)"}, ease: "power2.out"}]);
  }
  void outgoing;
  return null;
}
...
function compileOperation(operation) {
  ...
  if (operation.kind === "from_to") return `tl.fromTo(${JSON.stringify(operation.target)},${JSON.stringify(operation.from)},${JSON.stringify({...operation.to, duration: operation.duration_ms / 1000, ease: operation.ease})},${operation.start_ms / 1000});`;
...
function validateTransitionOperations(operations, {allowedTargets, sceneDurationMs, requireTrusted = false, compileContext} = {}) {
  ...
  for (const operation of operations) {
    ...
    if (operation.kind === "set") {
      ...
      exactKeys(operation, ["kind", "target", "start_ms", "duration_ms", "from", "to", "ease"], "transition_operation_invalid");
...
function validateTransitionStyle(style) {
  const allowed = new Set(["opacity", "clipPath"]);
  if (!plainRecord(style) || Object.keys(style).some((key) => !allowed.has(key))) throw new Error("transition_operation_invalid");
  for (const [key, value] of Object.entries(style)) {
    if (key === "opacity" && !Number.isFinite(value)) throw new Error("transition_operation_invalid");
    if (key === "clipPath" && !["inset(0 100% 0 0)", "inset(0 0% 0 0)"].includes(value)) throw new Error("transition_operation_invalid");
  }
}
