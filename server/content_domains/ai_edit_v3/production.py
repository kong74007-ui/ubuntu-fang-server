...
class QwenCompiledDirector:
    ...
    def _director_decision_system_prompt() -> str:
        ...
        return "\n".join((
            ...
            "For a repair, material_purpose_matches_selected_layout_semantic_slots means the target purpose must come from the selected layout's capabilities.layout_requirements semantic_slots. repair.constraint_envelope is authoritative timing and speaker-visibility data. For that material-purpose repair, when preserve_all_other_fields is true, change only the target_context purpose and preserve every other decision field.",
            "For a repair, visible_text_reference_object_or_omit means target_context.field must be either an object with only text_kind and source_caption_ids, using text_kind from target_context.allowed_text_kinds and IDs from target_context.allowed_source_caption_ids, or omitted when target_context.omission_allowed is true. When preserve_all_other_fields is true, the server accepts only that target field and freezes every other initial decision field.",
