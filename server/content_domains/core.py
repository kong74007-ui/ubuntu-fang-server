...
class H(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if p.startswith("/api/gen/") and p[9:] in HANDLERS:
            ...
            try:
                ...
                return self._send(503, {"detail": str(e)})
            try:
                body = self._json_body_strict() if kind in {"video", "tryon", "cinematic", "avatar", "ai_edit"} else self._json_body()
                ...
                idem_key = _idempotency_key(self.headers.get("Idempotency-Key")) if kind in {"video", "tryon", "xiaole_video", "cinematic", "ai_edit"} else ""
            except ValueError as e:
                return self._send(400, {"detail": str(e)[:220]})
    ...
    def do_GET(self):
        ...
        if p.startswith("/api/gen/job/"):
            ...
            phase = video_domain.get_video_job_phase(jid) if r["kind"] in {"video", "tryon", "xiaole_video", "cinematic", "ai_edit"} else None
