...
_REAL_FILES_DIR = os.path.realpath(FILES_DIR)

@app.get("/api/download/{filename:path}")
def download(filename: str):
    # 防路径穿越：只允许下载 FILES_DIR 内的文件（兼容 symlink）
    full = os.path.realpath(os.path.join(FILES_DIR, os.path.basename(filename)))
    if not full.startswith(_REAL_FILES_DIR + os.sep):
