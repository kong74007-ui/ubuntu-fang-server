...
- All 17 HyperFrames variants accept versioned AI semantic-boundary indices. At startup the service parses each variant's actual CSS font family, size, stroke, letter spacing, padding, effective text-box width, and available top layers; v02/v03 private-font overrides are then applied to those extracted metrics. The service verifies indices against the untouched source hash, rejects protected-word splits, and freezes the shortest balanced layout. Three-layer variants distribute detail copy across `top2` and `top3` while preferring complete punctuation boundaries. Requests without this optional contract retain the legacy layout path for rolling-deployment compatibility.
