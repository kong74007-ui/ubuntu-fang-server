...
- HyperFrames fixed-font overrides are server-owned template settings, not user-selectable inputs. `v03` replaces `top2` with the authorized `Smiley Sans Oblique` private font and increases only that layer to `58px`; its color, stroke, and line height remain unchanged. New jobs freeze the family/file/SHA/size mapping, stage only that extra font, and inject a task-local `@font-face`; already-admitted jobs without the frozen size keep their original template size.
