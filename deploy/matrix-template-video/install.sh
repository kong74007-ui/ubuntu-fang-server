...
GPU_HELPER_SOURCE="${DEPLOY_ROOT}/server/matrix_gpu_runtime.py"
...
for source in "${DEPLOY_ROOT}/server/matrix_motion_v3.py" \
  ...
  if [[ ! -f "${source}" || -L "${source}" || ! -r "${source}" ]]; then
    echo "missing or unsafe motion-v3 source: ${source}" >&2; exit 2
  fi
done
...
python3 "${REFERENCE_PALETTE_COMPAT_SOURCE}" --pack-root "${REFERENCE_PACK_ROOT}"
