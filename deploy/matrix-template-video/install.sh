...
NINE_GRID_ADAPTER_SHA256="45aad0e7f0af0d4a8dfa5b32effdccc8985c33e9f7dfa9e8842a5ea1349a9850"
...
for source in "${UNIT_SOURCE}" "${API_SOURCE}" "${LAYOUT_PATCH_SOURCE}" "${REFERENCE_LAYOUT_PATCH_SOURCE}" "${REFERENCE_V04_PREVIEW_CHECK_SOURCE}" "${NINE_GRID_ADAPTER_SOURCE}" "${ROLLBACK_LIB}"; do
...
env ONNXRUNTIME_NODE_INSTALL_CUDA=skip "${NODE_NPM}" install \
  --prefix "${NINE_GRID_RUNTIME}" --no-save --ignore-scripts --no-audit --no-fund \
  "hyperframes@${NINE_GRID_HYPERFRAMES_VERSION}"
