...
REMOTE_FORWARD_CHECK_SOURCE="${DEPLOY_ROOT}/deploy/pixelle-video/bin/check-novix-remote-forward-denied"
...
REMOTE_FORWARD_CHECK_TARGET="/usr/local/libexec/huangque/check-pixelle-novix-remote-forward-denied"
...
for source in "${RUNNER_SOURCE}" "${CHECK_SOURCE}" "${COMMAND_CHECK_SOURCE}" "${REMOTE_FORWARD_CHECK_SOURCE}" "${UNIT_SOURCE}"; do
...
install -o root -g root -m 0755 "${REMOTE_FORWARD_CHECK_SOURCE}" "${REMOTE_FORWARD_CHECK_TARGET}"
