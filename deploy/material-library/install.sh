...
for _ in $(seq 1 "${HEALTH_ATTEMPTS}"); do
  ...
  if EXPECTED_BUILD_ID="${BUILD_ID}" python3 -c \
      'import json,os,sys; d=json.load(sys.stdin); raise SystemExit(0 if d.get("ok") is True and d.get("build_id")==os.environ["EXPECTED_BUILD_ID"] and d.get("usage_state_ready") is True and type(d.get("selection_contract_version")) is int and d["selection_contract_version"]==2 and type(d.get("clip_contract_version")) is int and d["clip_contract_version"]==2 else 1)' \
      <<<"${response}"; then
    SUCCEEDED=1
