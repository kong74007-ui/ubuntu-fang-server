...
- Web：Fang Nginx 生效根目录固定为 `/var/www/html`。manifest 覆盖变化的 `site/workbench/` 页面与 `cloud-shell.js`、`site/register.html`、`site/admin/index.html`、`site/api-admin/index.html`；发布规范必须包含 `site/api-docs/openapi.json` -> `/var/www/html/api-docs/openapi.json`。字体与云支付图片依赖包含 `site/assets/fonts/` 下实际使用文件及 `site/assets/cloud/virtual-pay-item-200.png`。`docs/api/openapi.json` 是 repo-only 文档，不安装到 Web 根目录，但仍须与发布规范通过同一契约测试。
