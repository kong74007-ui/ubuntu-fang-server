...
- Phase A 不用有限关键词表冒充语义等价校验。在尚未引入由确定性原子事实跨度生成的可信压缩许可表前，`compressed` 只接受与引用准确文本 NFC 等价的内容；Qwen 需要缩短时改用更短的完整准确文本引用或 `ui_label`。后续放开真正措辞压缩必须升级协议和许可表，不能仅扩充否定词或承诺词词典。
...
Phase A 的初始布局变体只发布 `balanced_a`；初始主题只发布 `midnight_gold`、`editorial_sans` 以及 Schema 明确列出的密度、动效强度和图片适配 token。调用方传入 `null`、遗漏能力清单或增加未注册 token 均按不可信能力输入拒绝，不能回退为“全部允许”。后续增加变体或主题必须先发布新的版本化注册表并更新冻结 Schema/哈希。
...
本文没有授权修改 V2、合并 PR、部署测试站或生产站。书面规格复核完成后，由独立实施计划定义逐任务开发、测试和审查；进入测试部署与生产评审仍分别需要新的明确授权。
