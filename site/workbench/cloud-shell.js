...
(function(){
  ...
  var NAV=[
    ...
    {k:'video',l:'视频生成',i:'video'}, {k:'ai-edit',l:'一键剪辑',i:'edit'}, {k:'ai_edit_v2',l:'AI智能剪辑 V2',i:'edit',gated:true}, {k:'ai_edit_v3',l:'AI智能剪辑 V3',i:'edit',v3Gated:true}, {k:'audio',l:'音频生成',i:'mic'}, {k:'script',l:'文案编导',i:'edit'},
  ...
  var V3_NAV_PAGES={ai_edit_v3:'ai-edit-v3.html'};
  ...
  function navHTML(active){
    var admin=isAdmin();
    return NAV.filter(function(it){ return (admin || !it.admin) && (!it.gated || aiEditVisible) && (!it.v3Gated || aiEditV3Visible); }).map(function(it){
      var on=it.k===active;
      var ntxt=on?'#eaf1fa':'#94a4bb', nbg=on?'rgba(231,178,76,.08)':'transparent', nfg=on?'#e7b24c':'#94a4bb', nbar=on?'1':'0';
      return '<a href="'+escapeAttr(safeUrl(V3_NAV_PAGES[it.k]||NAV_PAGES[it.k]||it.k+'.html'))+'" class="hq-navitem" aria-label="'+escapeAttr(it.l)+'" data-nav-label="'+escapeAttr(it.l)+'" style="position:relative; display:flex; align-items:center; gap:12px; padding:10px 13px; border-radius:11px; cursor:pointer; color:'+ntxt+'; background:'+nbg+'; font-size:14px; font-weight:500; transition:.16s;">'+
  ...
  function build(){
    ...
    fetch('/api/v3/edit/capabilities',{credentials:'same-origin',cache:'no-store'})
      .then(function(r){return r.ok?r.json():null})
      .then(function(d){
        if(!d||!d.accepts_new_jobs) return;
        aiEditV3Visible=true;
        var nav=aside.querySelector('.hq-side-nav');
        if(nav){nav.innerHTML=navHTML(active);bindNavTooltips(aside);}
      }).catch(function(){});
